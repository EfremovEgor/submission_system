...
<article class="author-item ru_author">
	...
	<label class="prefix_field-container">
		...
		>
			...
			<option selected={'Mr.' == author.title} value="Mr.">Mr.</option>
			<option selected={'Mrs.' == author.title} value="Mrs.">Mrs.</option>
			<option selected={'Ms.' == author.title} value="Ms.">Ms.</option>
			<option selected={'Dr.' == author.title} value="Dr.">Dr.</option>
			<option selected={'Prof.' == author.title} value="Prof.">Prof.</option>
		</select>
	</label>
	<label>
	...
	<label>
		...
		>
			...
			{#each countryNames as country}
				<option selected={country == author.country} value={country}>{country}</option>
			{/each}
		</select>
	</label>
	<label>
	...
	<div class="author_checkbox-container">
		<label class="is_corresponding-label">
			Контактное лицо
			<input
				checked={author.id == 0 || author.is_corresponding}
				type="checkbox"
				on:input={(event) => {
		...
		<label class="is_presenter-label">
			...
			<input
				checked={author.id == 0}
				type="radio"
				value={author.id == 0 || author.is_presenter}
				name="is_presenter"
			/>
		</label>
	</div>
